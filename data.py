...
def build_dataset(
    ...
    if dataset_name == 'imagenet':
        ...
        STD = [0.229, 0.224, 0.225]
        NUM_CLASSES = 1000
        if train_transform_mode == "contrastive_pretrain":
            train_dataset = ImageNetIndex(root=os.path.join(data_path + "train"), transform=Transform())
            test_dataset = ImageNetIndex(root=os.path.join(data_path + "val"), transform=Transform())
            print("byol aug")            
        else:
